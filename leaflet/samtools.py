...
default_dgram_api = ('127.0.0.1', 7656)
...
class HandshakeError(pysocket.error):
    def __init__(self, msg):
        super().__init__(errno.ENOTCONN, msg)

class NSError(pysocket.error):
    def __init__(self, msg):
        super().__init__(errno.EAGAIN, msg)

class CreateDestError(pysocket.error):
    def __init__(self, msg):
        super().__init__(errno.EADDRNOTAVAIL, msg)

class ReachError(pysocket.error):
    def __init__(self, msg):
...
def lookup(sock, domain, cache = None):
    ...
    if b64_dest:
        ...
        raise NSError('Domain name %r not resolved because %r' % (domain, reply.result))


def session_create(sock, sock_type, sig_type, nickname, i2cp_options = None):
    i2cp = i2cp_options or {}
    if sock_type == 'stream':
        style = 'STREAM'
    elif sock_type == 'datagram':
        style = 'DATAGRAM'
        if 'PORT' not in i2cp:
            raise ValueError('Must specify a port to forward datagrams.')
    else:
        raise NotImplementedError()

    cmd = 'SESSION CREATE STYLE=%s DESTINATION=TRANSIENT SIGNATURE_TYPE=%d ID=%s ' % (style, sig_type, nickname)
    cmd += ' '.join('%s=%s' % (k, v) for (k, v) in i2cp.items())
